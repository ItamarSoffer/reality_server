...
def fetch_event_extra_data(event_data, username):
    ...
    logging.info("process is done by {}".format(mp.current_process().name))
    ...
    if extra_data:
        logging.info("MATCHED! {}".format(event_data['link']))
...
def fetch_story_extra_data(events, username):
    ...
    for event_data in events:
        if event_data['link']:
            ...
            # logging.info('{}'.format(time.perf_counter() - start))
        else:
            no_extra.append(event_data)
    logging.info(' all started: {}'.format(time.perf_counter() - start))

    output = [p.get() for p in mp_results]
    pool.close()

    end = time.perf_counter()
    logging.info("all ended: {} seconds".format(end - start))
